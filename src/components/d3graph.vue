  ...
  <div style="width:100%">
    ...
    <div id="mode">
      ...
      <div class="gState" style="margin-bottom: 20px;">
        <span @click="changeTextState(0)" :class="{ active: isShowText }"
        ...
        >关系(度)</span> -->
        <span @click="changeTextState(2)" :class="{ active: textState === 2 }"
    ...
    <div id="indicator">
      ...
      <div v-for="(name, index) in names" :key="index">
        <span @click="hideNodeOfType" :data-state="states[index]" :data-index="index" style="cursor: pointer;"
    ...
    <div id="info" v-show="selectNodeData.name !== undefined">
      ...
      <el-card :style="{ backgroundColor: selectNodeData.color }" class="node-card">
        <div slot="header" class="clearfix">
          ...
          <!-- 上面这个编辑按钮的功能暂时不做，先把查询该节点的能力做了 -->
          <!-- <el-button @click="queryNode" style="float: right; padding: 3px 0;color: #409EFB;font-size: 15px;"
...
export default {
  ...
  data() {
    return {
      ...
      colors: ['#FFC0CB', '#FFA07A', '#FFD700', '#98FB98', '#00FFFF', '#FF69B4', '#9370DB', '#00FA9A', '#1E90FF', '#FF8C00', '#7CFC00', '#FF00FF', '#6A5ACD', '#FFE4B5', '#DC143C', '#808080', '#FF1493', '#008B8B'],
      states: [],
      ...
      menu: [
        {
          ...
          action: (elm, d) => {
            ...
            /* 将数据传回父组件，以便于交给search查询 */
            this.$emit('queryNode', d.properties.name)
            this.d3render() // 重新渲染图
            this.stateInit()
          },
  ...
  methods: {
    ...
    queryNode() {
      ...
      let node_name = this.selectNodeData.name
      console.log(node_name);
      /* 将数据传回父组件，以便于交给search查询 */
      this.$emit('queryNode', node_name)
